...
class XIcon : ImageView, ViewInterface {
    ...
    override fun extraTypes(typedArray: TypedArray) {
        mRadius = ViewHelper.getDimension( typedArray, R.styleable.XIcon_cornerRadius, 0f)
        mImagePath = ViewHelper.getString( typedArray, R.styleable.XIcon_imagePath)
        mImageDefault = ViewHelper.getResourceId( typedArray, R.styleable.XIcon_imageDefault,0)
        mAllowCache = ViewHelper.getBoolean( typedArray, R.styleable.XIcon_allowCache,false)
        mTint = ViewHelper.getColorStateList( typedArray, R.styleable.XIcon_android_tint)
    }
