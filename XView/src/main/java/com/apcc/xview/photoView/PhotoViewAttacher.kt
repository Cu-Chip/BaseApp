...
import com.apcc.xview.utils.Util

class PhotoViewAttacher : OnTouchListener,
    ...
    override fun onTouch(v: View, ev: MotionEvent): Boolean {
        ...
        if (mZoomEnabled && Util.hasDrawable(v as ImageView)) {
            ...
            mScaleDragDetector?.let {scaleDD->
                val wasScaling: Boolean = scaleDD.isScaling()
                val wasDragging: Boolean = scaleDD.isDragging()
                handled = scaleDD.onTouchEvent(ev)
                val didntScale = !wasScaling && !scaleDD.isScaling()
                val didntDrag = !wasDragging && !scaleDD.isDragging()
    ...
    private fun setImageViewMatrix(matrix: Matrix) {
        ...
        mMatrixChangeListener?.let { mtls->
            val displayRect = getDisplayRect(matrix)
            if (displayRect != null) {
                mtls.onMatrixChanged(displayRect)
            }
        }
    }
