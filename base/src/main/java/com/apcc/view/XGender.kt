...
class XGender: ConstraintLayout, ViewInterface {
    ...
    override fun extraTypes(typedArray: TypedArray) {
        mGender = ViewHelper.getInteger( typedArray, R.styleable.XGender_gender, Gender.UNISEX)
        mLabel = ViewHelper.getString( typedArray, R.styleable.XGender_android_label)
        mEnable = ViewHelper.getBoolean( typedArray, R.styleable.XGender_android_enabled, true)
    }

    override fun initView(root: View) {
        lblLabel = root.findViewById(R.id.apccLabel)
        rabUnisex = root.findViewById(R.id.apccUnisex)
        rabMale = root.findViewById(R.id.apccMale)
        rabFemale = root.findViewById(R.id.apccFemale)
        ragGroup = root.findViewById(R.id.apccGroup)
