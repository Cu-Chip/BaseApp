                ...
                event?.let { ev ->
                    when (ev.action) {
                        MotionEvent.ACTION_DOWN, MotionEvent.ACTION_POINTER_DOWN -> {
                            Log.d("XCrop", "action down ${ev.x} ${ev.y}")
                            targetCropPoint(ev.x, ev.y)
                        }
                        MotionEvent.ACTION_MOVE -> {
                            Log.d("XCrop", "action move")
                            if (moveFocusPoint(ev.x, ev.y)) {
                                hasMove = true
                            }
                        }
                        MotionEvent.ACTION_UP, MotionEvent.ACTION_POINTER_UP -> {
                            Log.d("XCrop", "action up")
        ...
        canvas?.let { cv->
            startPoint?.let { stPoint->
                endPoint?.let { edPoint->
                    ...
                    Log.d("FUCK", "st: ${startPoint.x} : ${startPoint.y} \n ed: ${endPoint.x} : ${endPoint.y} \n point: ${highlightPointStart.x} : ${highlightPointStart.y}")
                }
            }
        }
    }
...
//        bottom = max(startY, endY)
    ...
    interface DrawingListener{
        ...
        fun onBitmapDrawArea( rect:Rect )
        fun onNotifyState(hasSelectArea:Boolean, hasSelectPoint:Boolean)
    }
